import 'package:flutter/material.dart';
import 'package:rmtools/pages/tela_poderesmenu.dart';
import 'package:rmtools/model/fichaModel/armazenamento_ficha.dart';

class TelaFichabasica extends StatefulWidget {
  final String nomePersonagem;
  const TelaFichabasica({super.key, required this.nomePersonagem});

  @override
  State<TelaFichabasica> createState() => _TelaFichabasicaState();
}

class _TelaFichabasicaState extends State<TelaFichabasica> {
  final nomePoder = TextEditingController();
  
  bool erro = false;
  bool sucesso = false;

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      resizeToAvoidBottomInset: false,
      backgroundColor: const Color.fromARGB(255, 21, 22, 34),
      body: Stack(
        alignment: Alignment.center,
        children: [
          Align(
            alignment: Alignment.topCenter,
            child: Padding(
              padding: const EdgeInsets.only(top: 20),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                
                
                //***Titulo***
                children: [
                  const Text(
                    "Poder",
                    style: TextStyle(
                      fontSize: 35,
                      color: Colors.white,
                    ),
                  ),

                  
                  //***Espaçamento***
                  const SizedBox(height: 40),
                  
                  
                  //***Campo Nome Poder***
                  SizedBox(
                    width: 300,
                    height: 100,
                    child: TextField(
                      controller: nomePoder,
                      style: const TextStyle(color: Colors.white),
                      decoration: const InputDecoration(
                        labelText: "Nome do Poder",
                        border: OutlineInputBorder(),
                        contentPadding:
                            EdgeInsets.symmetric(vertical: 20, horizontal: 10),
                      ),
                    ),
                  ),

                  //***Padding dos botões***
                  Padding(
                    padding: const EdgeInsets.only(bottom: 30),

                    //***Botao Cria Poder***
                    child: ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        minimumSize: const Size(300, 100),
                      ),
                      onPressed: () async{
                        final navigator = Navigator.of(context);

                        if (nomePoder.text.isEmpty){
                          setState(() {
                            erro = true;
                            sucesso = false;
                          });
                          return;
                        }

                        setState(() {
                          erro = false;
                          sucesso = false;

                          //Salva o poder
                          final repo = FichaRepository();
                          repo.salvarPoder(widget.nomePersonagem, nomePoder.text);
                          
                          sucesso = true;
                        });

                        await Future.delayed(const Duration(seconds: 1));

                        if(!mounted){
                          return;
                        }

                        navigator.push(
                            MaterialPageRoute(
                              builder: (context) => TelaPoderesMenu(nomePersonagem: widget.nomePersonagem),
                            )
                        );   
                      },
                      child: const Text(
                        "Criar ficha",
                        style: TextStyle(fontSize: 25),
                      ),
                    ),
                  ),


                  //***Botao Voltar***
                  Padding(
                    padding: const EdgeInsets.only(bottom: 30),
                    child: ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        minimumSize: const Size(100, 50),
                      ),
                      onPressed: () {
                        Navigator.pop(
                          context,
                          MaterialPageRoute(
                            builder: (context) => TelaPoderesMenu(nomePersonagem: widget.nomePersonagem),
                          ),
                        );
                      },
                      child: const Text(
                        "Voltar",
                        style: TextStyle(fontSize: 25),
                      ),
                    ),
                  ),
                ],
              ),
            ),
          ),

          //erro
          if (erro)
            Container(
              padding: const EdgeInsets.all(16),
              child: const Text(
                "Preencha todos os dados!",
                style: TextStyle(
                  color: Color.fromARGB(255, 255, 0, 0),
                  fontSize: 25,
                  fontWeight: FontWeight.bold,
                ),
              ),
            ),

          //sucesso
          if (sucesso)
            Container(
              padding: const EdgeInsets.all(16),
              child: const Text(
                "Ficha criada com sucesso!",
                style: TextStyle(
                  color: Color.fromARGB(255, 0, 255, 0),
                  fontSize: 25,
                  fontWeight: FontWeight.bold,
                ),
              ),
            ),
        ],
      ),
    );
  }
}
